...
minErr.1se <- which.min(cpTable[, 4])  # column 4 is xerror

# Extracts the cp parameter corresponding to the 1se rule
tree.fit.1se <- rpart::prune(rpart.class.tree.fit, 
                             cp = cpTable[minErr.1se, 1])  # column 1 is cp


# Cp Table and cp plot for the 1se tree
cpTable <- printcp(rpart.class.tree.fit)
plotcp(tree.fit.1se)
...
In this case, the **selected tree has 1 split, with 2 terminal nodes** (cp=0.0150376, xerror=0.19549). Therefore, the tree selected by the 1SE rule is the same as the one selected by the minimum cross-validation error, and they have the same tree size. 
...
To find the test performance for this model, I computed the AUC (area under the ROC curve). Based on the plot, **AUC was 0.986**, which represents excellent classification performance on the test set.
